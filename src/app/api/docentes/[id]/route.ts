    ...
    try {
        ...
        if (error.response) {
            ...
            return NextResponse.json(
                { error: "El servidor ha respondido con un código de estado de error" },
                {
            ...
            return NextResponse.json(
                { error: "La solicitud no pudo completarse debido a un problema de red" },
                {
                    status: 500,
                }
            );
            ...
            if (error.code === "ER_ROW_IS_REFERENCED_2") {
                return NextResponse.json(
                    { error: "No se puede eliminar este docente porque está en uso" },
                    {
                        status: 409,
                    }
                );
                ...
                return NextResponse.json(
                    { error: "Ha ocurrido un error al intentar eliminar el docente" },
                    {
                        status: 500,
                    }
                );
